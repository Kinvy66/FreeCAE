...
FCLegendScaleActor::FCLegendScaleActor()
{
    ...
    this->BottomAxis->SetVisibility(0);
    ...
    legendLinesBd->AllocateEstimate(4, 2);
#endif
...
void FCLegendScaleActor::BuildRepresentation(vtkViewport* viewport)
{
    ...
    this->BottomAxis->GetPositionCoordinate()->SetValue(x1, 0, 0.0);
    this->BottomAxis->GetPosition2Coordinate()->SetValue(size[0] - x1, 0, 0.0);
    ...
    double d = std::sqrt(vtkMath::Distance2BetweenPoints(xL, xR));

    char buf0[256], buf1[256], buf2[256], buf3[256], buf4[256];
    ...
    this->LegendPoints->SetPoint(0, x1, 30, 0.0);
    ...
    this->LegendPoints->SetPoint(4, x1 + 4 * delX, 30, 0.0);
    this->LegendPoints->SetPoint(5, x1, 40, 0.0);
    this->LegendPoints->SetPoint(6, x1 + delX, 40, 0.0);
    this->LegendPoints->SetPoint(7, x1 + 2 * delX, 40, 0.0);
    this->LegendPoints->SetPoint(8, x1 + 3 * delX, 40, 0.0);
    this->LegendPoints->SetPoint(9, x1 + 4 * delX, 40, 0.0);
    ...
    for (int i = 0; i < 5; i++)
    {
        double* pt = this->LegendPoints->GetPoint(i);
        this->LabelActors[i]->SetPosition(pt[0], pt[1] - 1);
    }
